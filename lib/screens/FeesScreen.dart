...
import 'package:intl/intl.dart';
import 'package:vidhyatra_flutter/controllers/PaymentController.dart';
import 'package:vidhyatra_flutter/controllers/FeeController.dart';
import 'package:vidhyatra_flutter/controllers/PaymentHistoryController.dart';
...
class FeesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Helper function to get warning message based on payment status and due date
    Map<String, dynamic> getWarningMessage(Fee fee) {
      // Find the payment history entry for this fee
      final payment = paymentHistoryController.paymentHistory.firstWhereOrNull(
            (p) => p.feeId == fee.feeID,
      );

      // If payment is completed, no warning message
      if (payment != null && payment.status.toLowerCase() == 'completed') {
        return {'show': false, 'message': '', 'color': Colors.transparent, 'icon': Icons.check};
      }
      ...
      if (isOverdue) {
        return {
          'show': true,
          'message': 'The due date has passed. Please visit the Finance Department to settle your fee.',
          'color': Colors.red,
        ...
        return {
          'show': true,
          'message': 'Just $daysRemaining day(s) left! Make sure to complete your payment on time.',
        ...
        return {
          'show': true,
          'message': 'Payment pending. Please complete your payment before the due date.',
          'color': Colors.amber,
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          Obx(() {
            if (feeController.isLoading.value || paymentHistoryController.isLoading.value) {
              return Center(
                child: CircularProgressIndicator(
                  color: const Color(0xFF186CAC),
                ),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          Fee fee = feeController.fees[index];
                          final warning = getWarningMessage(fee);
                                          ...
                                          Row(
                                            children: [
                                              Icon(
                                                Icons.calendar_today,
                                                size: 18,
                                                color: warning['color'] == Colors.red ? Colors.red : Colors.grey[600],
                                              ),
                                              SizedBox(width: 8),
                                              Text(
                                                "Due: ${DateFormat('yyyy-MM-dd').format(DateTime.parse(fee.dueDate))}",
                                                style: GoogleFonts.poppins(
                                                  fontSize: 14,
                                                  fontWeight: FontWeight.w500,
                                                  color: warning['color'] == Colors.red ? Colors.red : Colors.grey[700],
                                                ),
                                              ),
                                            ],
                                          ),
            ...
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (feeController.selectedFee.value != null) {
                  Fee selectedFee = feeController.selectedFee.value!;
                  final payment = paymentHistoryController.paymentHistory.firstWhereOrNull(
                        (p) => p.feeId == selectedFee.feeID && p.status.toLowerCase() == 'completed',
                  );
                  if (payment != null) {
                    Get.snackbar(
                      "Error",
